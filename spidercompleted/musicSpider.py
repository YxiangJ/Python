...
def get_channel_list(page_url):
    try:
        htmlDoc = urlopen(page_url).read().decode('utf-8')
...
def get_song_list(channel_url):
    try:
        htmlDoc = urlopen(channel_url).read().decode('utf-8')
    except Exception as e:
        return {}
...
def get_song_rel_url(song_url):
    try:
        htmlDoc = urlopen(song_url).read().decode('utf-8')
        # print(htmlDoc)
    except Exception as e:
        return (None, None, 0)
...
def donwn_mp3_by_link(song_link, song_name, song_size):
    ...
    while True:
        try:
            ...
            if down_loaded_size >= song_size:
                print('%s download finished' % file_full_path)
                break
        except Exception as e:
            if os.path.getsize(file_full_path):
                if os.path.exists(file_full_path):
                    os.remove(file_full_path)
                    print('download time out, file deleted')
                    with open('log.txt', 'a') as log_file:
...
def downViaMutiThread(song_info_list):
    ...
    for song_name, song_link, song_size in song_info_list:
        t = threading.Thread(target=donwn_mp3_by_link,
                             args=(song_link, song_name, song_size))
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for song_id in song_id_list:
            ...
            song_name, song_link, song_size = get_song_rel_url(song_url)
            if song_size != 0:
